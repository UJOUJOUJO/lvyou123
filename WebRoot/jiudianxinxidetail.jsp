...
<table width="978" height="1081" border="0" align="center" cellpadding="0" cellspacing="0" id="__01">
  ...
  zys=zgs/page_record+1;
   ...
   sql="";
  sql="select top "+page_record+" * from kefang where jiudian='"+mingcheng+"' and id not in (select top "+((curpage-1)*page_record)+" id from kefang where jiudian='"+mingcheng+"' order by id desc) ";
...
%>
        <tr>
          ...
          <td><%=jiage %></td>
          <td><%=beizhu %></td>
          <td width="60" align="center"><a href="jiudianyudingadd.jsp?jd=<%=jiudian%>&kfbh=<%=kefangbianhao%>&jg=<%=jiage%>">预订</a><a href="del.jsp?id=<%=id %>&tablename=kefang" onClick="return confirm('真的要删除？')"></a></td>
        </tr>
        <%
  }
   %>
